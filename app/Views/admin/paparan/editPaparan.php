...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Edit Files Paparan</h6>
        </div>
        <div class="card-body">
            <form action="<?= base_url('admin/files/paparan/update') ?>" method="POST" enctype="multipart/form-data">
                <?= csrf_field() ?>
                ...
                <div class="row">
                    <div class="col-12 col-md-8">
                        ...
                        <div class="form-group">
                            <label for="link">File Paparan</label>
                            <div>
                                <?php $no = 1;
                                foreach ($files as $f) : ?>
                                    <input type="hidden" name="id[]" value="<?= $f->id ?>">
                                    <label for="">Paparan <?= $no++ ?></label>
                                    <div class="custom-file mb-2 <?php if (session('errors.paparan')) : ?>is-invalid<?php endif ?>">
                                        <input type="file" name="paparan[]" class="custom-file-input" id="customFile">
                                        <label class="custom-file-label" for="customFile"><?= $f->nama ?></label>
                                    </div>
                                    <div class="mb-3">
                                        <a href="<?= base_url('uploads/berkas/paparan') . '/' . $f->nama ?>" target="__BLANK">Lihat Paparan</a>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                            <?php if (session('errors.paparan')) : ?>
                                <div id="validationServer03Feedback" class="invalid-feedback">
                                    <?= session('errors.paparan') ?>
                                </div>
                            <?php endif; ?>
                        </div>
